...
description: >
  ...
  as scarce "voltage", hairline ink elevation (no heavy shadow), __nextjs-Geist for all
  display + __nextjs-Geist body + __nextjs-Geist Mono for the index/code voice on a warm-navy code surface,
...
typography:
  # — reading + chrome ramp —
  body:    { fontFamily: "__nextjs-Geist", cqw: 1.5, weight: 400, lineHeight: 1.5 }
  lead:    { fontFamily: "__nextjs-Geist", cqw: 2.08, weight: 400, lineHeight: 1.5 }
  card-title:{ fontFamily: "__nextjs-Geist", cqw: 2.3, weight: 600, lineHeight: 1.25, tracking: "-0.005em" }
  button:  { fontFamily: "__nextjs-Geist", cqw: 1.46, weight: 600, lineHeight: 1.0 }
  tag-upper:{ fontFamily: "__nextjs-Geist", cqw: 1.35, weight: 600, tracking: "0.18em", upper: true }
  kicker:  { fontFamily: "__nextjs-Geist Mono", px: 28, cqw: 1.46, weight: 600, tracking: "0.16em", upper: true }
  mono-label:{ fontFamily: "__nextjs-Geist Mono", px: 26, cqw: 1.35, weight: 600, tracking: "0.02em" }
  code:    { fontFamily: "__nextjs-Geist Mono", cqw: 1.67, weight: 400, lineHeight: 1.6 }
  # — display ramp (__nextjs-Geist 400, sentence case, negative tracking. Renderer embeds only 400/700 — author at 400; italic is the synthesized slant) —
  headline:{ fontFamily: "__nextjs-Geist", cqw: 4.6, weight: 400, lineHeight: 1.06, tracking: "-0.018em" }
  quote-pull:{ fontFamily: "__nextjs-Geist", cqw: 5.0, weight: 400, lineHeight: 1.12, tracking: "-0.012em", italic: true }
  display-italic:{ fontFamily: "__nextjs-Geist", cqw: 6.7, weight: 400, lineHeight: 1.05, tracking: "-0.012em", italic: true }
  display:{ fontFamily: "__nextjs-Geist", cqw: 7.3, weight: 400, lineHeight: 1.02, tracking: "-0.022em" }
  number-hero:{ fontFamily: "__nextjs-Geist", cqw: 9.4, weight: 400, lineHeight: 0.95, tracking: "-0.025em" }
  display-cover:{ fontFamily: "__nextjs-Geist", cqw: 9.9, weight: 400, lineHeight: 0.98, tracking: "-0.028em" }
  number-unit:{ fontFamily: "__nextjs-Geist Mono", cqw: 2.08, weight: 600, lineHeight: 1.0 }
...
components:
  ...
  kicker-spike:
    ...
    description: "The eyebrow — __nextjs-Geist Mono uppercase, indexical (2–5 words), prefixed with the coral ✱. Never plain text, never a sentence."
  coral-callout:
  ...
  number-lockup:
    ...
    description: "Hero stat — a __nextjs-Geist figure paired with a __nextjs-Geist Mono unit (200K, +1,204, −318, 17 files). Figure is serif; the unit is ALWAYS mono, never the serif."
  pull-quote:
    typography: "{typography.quote-pull} (__nextjs-Geist italic) + {typography.tag-upper} cite"
    description: "A commit message, a reviewer line, or the thesis. __nextjs-Geist italic, small __nextjs-Geist uppercase cite beneath."
  section-rule:
  ...
  code-surface:
    ...
    textColor: "{colors.cream} (__nextjs-Geist Mono); syntax in coral (keywords) / teal #5DB8A6 (strings) / amber #E8A55A (numbers)"
...
- **Fonts** — already set to **__nextjs-Geist** (display) / **__nextjs-Geist Mono** (body); ignore any preset font name lingering in prose.
- **Weights** — the brand font ships `{400, 600, 620, 650, 680, 700}` only; every weight is clamped to these — ignore higher preset weights (e.g. 600/700) in prose.
...
Three editorial voices, each in its own face: **__nextjs-Geist** carries every display moment — covers,
headlines, pull-quotes, big stat numerals — at large display sizes with gentle negative tracking;
its **italic** is the expressive register. **__nextjs-Geist** carries body, leads, card titles, buttons, and
UI chrome. **__nextjs-Geist Mono** carries the indexical layer — kickers, technical labels, the code
...
- **__nextjs-Geist** (sentence case, negative-tracked) for all display; **__nextjs-Geist** body/chrome; **__nextjs-Geist Mono** index + code.
...
- **Squint** — one __nextjs-Geist display moment dominates at 3–6× its neighbor.
...
- **Type** — __nextjs-Geist sentence-case display (negative-tracked); __nextjs-Geist 400 body; __nextjs-Geist Mono kickers (uppercase 0.16em, coral ✱) + code.
...
Two ramps. The **reading/chrome ramp** (__nextjs-Geist `body` 1.5cqw / `lead` 2.08cqw weight 400; JetBrains
Mono `kicker`/`mono-label` in px) carries copy + chrome; the **display ramp** (__nextjs-Geist `headline` 4.6cqw
...
- **__nextjs-Geist display is sentence case** (NOT title case, NOT uppercase), weight 400, negative-tracked (−0.012..−0.028em); reach for **italic** when the line is a stance or a definition. **__nextjs-Geist body** sentence case weight 400. **__nextjs-Geist Mono** kickers UPPERCASE 0.16em with the coral ✱. No uppercase serif, no sans headline, no serif label.
...
- **number-lockup** — __nextjs-Geist figure + mono unit (the PR `+N / −M`, `200K`, file counts). **pull-quote** — __nextjs-Geist italic + cite (a commit message / reviewer line).
...
### 1 · Cover (identity · move: oversized __nextjs-Geist · cream)

**Ground** `{colors.cream}`, `slide-pad`. **Composes** kicker-spike, display-cover, lead, mono-label index. **Focal** a 2–3 line __nextjs-Geist `display-cover` (sentence case, ink) under a coral ✱ kicker. **Chrome** mono index strip (repo · branch). **Accent** the single coral ✱. **Fixed** __nextjs-Geist 400 sentence case, hairline, cream ground. **Free** title, the mono index, layout + how full the frame runs. **Density** free.

### 2 · Statement (statement · move: single __nextjs-Geist line · cream or navy)

**Ground** `{colors.cream}` (or `{colors.navy}` for gravity). **Composes** kicker-spike, display, optional lead. **Focal** one 2-line __nextjs-Geist `display` carrying the change in a sentence — reach for **italic** if it's a stance. **Chrome** mono kicker. **Accent** none — the serif carries it (or one coral word). **Fixed** sentence-case serif. **Free** the line, ground, layout + density. **Density** free.
...
**Ground** `{colors.cream}`, `slide-pad`. **Composes** kicker-spike, number-lockup, lead/caption, optional `section-rule`. **Focal** a __nextjs-Geist `number-hero` figure with a mono unit over a 1px rule — the PR impact (`+1,204 / −318`, `17 files`, `2.1× faster`). **Chrome** mono tag. **Accent** the figure in ink; at most one coral unit. **Fixed** serif figure + mono unit, hairline rule. **Free** the figures (from the script), tag, layout + density. **Density** free.

### 5 · Pull-quote (quote · move: __nextjs-Geist italic · cream)

**Ground** `{colors.cream}`. **Composes** kicker-spike, pull-quote, tag-upper cite. **Focal** a __nextjs-Geist **italic** `quote-pull` — a commit message, a reviewer line, or the thesis — with a small __nextjs-Geist uppercase cite (author · role) beneath. **Chrome** mono kicker. **Accent** none, or one coral mark. **Fixed** __nextjs-Geist italic quote + uppercase cite. **Free** quote, attribution, layout + density. **Density** free.

### 6 · Closing / CTA (closer · move: coral voltage · cream or navy)

**Ground** `{colors.cream}` (or `{colors.navy}`). **Composes** display sign-off, coral-callout, optional contributor row (`assets/<login>.png` avatars + mono names). **Focal** a short __nextjs-Geist sign-off with the one **coral-callout** (the CTA or full-bleed band) and, for a "shipped-by" close, a row of hairline-ringed avatar chips. **Chrome** mono index. **Accent** the single coral voltage. **Fixed** one coral moment, hairline avatar rings, sentence-case serif. **Free** sign-off, who ships, layout + density. **Density** free.
...
- Set all display in **__nextjs-Geist, sentence case**, negative-tracked; **__nextjs-Geist 400** body; **__nextjs-Geist Mono** kickers (uppercase, 0.16em, coral ✱) + code.
...
- Pair a __nextjs-Geist figure with a **mono unit** for every stat; render code via the **`code-*` blocks** on the navy surface.
...
- No uppercase or title-case __nextjs-Geist display; no sans headline; no serif label; no serif-set numeric unit.
...
- **Squint** — one __nextjs-Geist display moment dominates.
...
- **Type** — __nextjs-Geist sentence-case display (negative-tracked); __nextjs-Geist 400 body; __nextjs-Geist Mono kickers (uppercase 0.16em, coral ✱) + code; ≥1.4cqw floor.
